...
public class ShanghaiPageProcessor implements PageProcessor,Job {
...
//	public static final String URL_POST = "http://rexian\\.beijing\\.gov\\.cn/default/com\\.web\\.consult\\.consultDetail\\.flow\\?originalId=[a-zA-z0-9]+";
	private static Logger logger= Logger.getLogger(ShanghaiPageProcessor.class) ;
	...
	public static void main(String[] args) {
		...
		Spider addPipeline = Spider.create(new ShanghaiPageProcessor()).addPipeline(new ShanghaiPipleline());
		Spider addUrl = null;
		Integer totalPage = SHTotalPageUtilProcessor.getTotalPage("http://zwdt.sh.gov.cn/zwdtSW/bsfw/showItemList.do?pageSize=10&pageNumber=1&ST_ORG_REGION=SH00SH&ST_ITEM_FNAME=all&ST_ITEM_TYPE=all&ST_NET=&ST_FEATURE=&ST_KEYWORD=&SORT=DEFAULT&ST_ITEM_CODE=&GFType=g");
		for (int i=1;i<=totalPage;i++) {
			 addUrl = addPipeline.addUrl("http://zwdt.sh.gov.cn/zwdtSW/bsfw/showItemList.do?pageSize=10&pageNumber="+String.valueOf(i)+"&ST_ORG_REGION=SH00SH&ST_ITEM_FNAME=all&ST_ITEM_TYPE=all&ST_NET=&ST_FEATURE=&ST_KEYWORD=&SORT=DEFAULT&ST_ITEM_CODE=&GFType=g");
		}
